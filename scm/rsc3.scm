; AUTOGENERATED: Wed 09 Jun 2021 15:37:35 AEST
...
(define n-set (lambda (i xys) (let ((z (concatMap (lambda (xy) (list (car xy) (cdr xy))) xys))) (message "/n_set" (cons i z)))))
...
(define Splay
  (lambda (i s l c lc)
    (let* ((n (if (mce? i) (max 2 (mceDegree i)) 2))
      ...
      (Mix (Pan2 i (MulAdd s (asMce p) c) (Mul l a))))))
                 ...
                 (if (> n 0)
                     (begin
                       ...
                       (let* ((u* (from-maybe-closure u))
                              (u** (if (not s) (Out 0 u*) (with-env u* 1 s t removeSynth))))
                         (play-at fd u** -1 addToHead 1))
